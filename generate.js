...
function postUrl(slug) { return `${SITE_URL}/posts/${encodeURIComponent(slug)}/`; }
...
function getSidebarHTML(posts, categories, activeCategory = null) {
  const recent = posts.slice(0, 5).map((p) => {
    const m = getPostMeta(p);
    return `<li><a href="/posts/${encodeURIComponent(m.slug)}/"><span>${escapeHTML(m.title)}</span><small>${escapeHTML(formatDate(m.date))}</small></a></li>`;
  }).join("");
...
function generateIndexPage(posts) {
  ...
  const cards = posts.map((post) => {
    ...
    return `<article class="post-card"><a href="/posts/${encodeURIComponent(m.slug)}/" aria-label="Read ${escapeAttr(m.title)}">${m.coverImage ? `<div class="post-card-image"><img src="${escapeAttr(m.coverImage)}" alt="${escapeAttr(m.title)}" loading="lazy" decoding="async"></div>` : ""}</a><div class="post-card-body">${m.categories.slice(0,2).map(c=>`<span class="post-category">${escapeHTML(c)}</span>`).join("")}<h2><a href="/posts/${encodeURIComponent(m.slug)}/">${escapeHTML(m.title)}</a></h2><p class="post-card-summary">${escapeHTML(m.summary)}</p><div class="post-card-footer"><span class="post-date">${escapeHTML(formatDate(m.date))}</span><a class="read-more" href="/posts/${encodeURIComponent(m.slug)}/">Read →</a></div></div></article>`;
  }).join("");

  const featuredHTML = featured ? `<article class="featured"><div class="featured-image">${featuredMeta.coverImage ? `<img src="${escapeAttr(featuredMeta.coverImage)}" alt="${escapeAttr(featuredMeta.title)}">` : ""}</div><div class="featured-copy"><div class="section-kicker">Featured insight</div>${featuredMeta.categories.slice(0,2).map(c=>`<span class="post-category">${escapeHTML(c)}</span>`).join("")}<h2><a href="/posts/${encodeURIComponent(featuredMeta.slug)}/">${escapeHTML(featuredMeta.title)}</a></h2><p class="featured-summary">${escapeHTML(featuredMeta.summary)}</p><div class="meta"><span>${escapeHTML(formatDate(featuredMeta.date))}</span><span>•</span><span>${readingTime(featuredMeta.summary)} min read</span></div><a class="read-more" href="/posts/${encodeURIComponent(featuredMeta.slug)}/">Read the insight →</a></div></article>` : "";
...
function generateCategoryPage(category, posts, categories) {
  ...
  const cards = filtered.map(p=>{const m=getPostMeta(p);return `<article class="post-card"><a href="/posts/${encodeURIComponent(m.slug)}/">${m.coverImage?`<div class="post-card-image"><img src="${escapeAttr(m.coverImage)}" alt="${escapeAttr(m.title)}" loading="lazy"></div>`:""}</a><div class="post-card-body">${m.categories.map(c=>`<span class="post-category">${escapeHTML(c)}</span>`).join("")}<h2><a href="/posts/${encodeURIComponent(m.slug)}/">${escapeHTML(m.title)}</a></h2><p class="post-card-summary">${escapeHTML(m.summary)}</p><div class="post-card-footer"><span class="post-date">${escapeHTML(formatDate(m.date))}</span><a class="read-more" href="/posts/${encodeURIComponent(m.slug)}/">Read →</a></div></div></article>`}).join("");
...
async function generate(){
  ...
  fs.rmSync(dist,{recursive:true,force:true}); fs.mkdirSync(postsDir,{recursive:true}); fs.mkdirSync(catDir,{recursive:true}); fs.cpSync(path.join(__dirname,"assets"),path.join(dist,"assets"),{recursive:true});
